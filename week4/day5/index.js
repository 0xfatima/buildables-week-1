...
const mianInput = document.querySelector("#todo-form-input")
...
todoForm.addEventListener('submit', (e)=>{
    e.preventDefault()
    const inputValue = mianInput.value
    if(inputValue==''){
        return
    }
...
e.target.parentElement.parentElement.classList.contains('remove-task')){
        const taskId = e.terget.closest('li').id
        removeTask(taskId)
    }
})
...
function removeTask(taskId){
    ...
    document.getElementsById(taskId).remove()
    countTasks()
}
